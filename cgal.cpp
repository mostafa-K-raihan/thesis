...
double computeLikelihood(const char *file) {
	...
	int fileCount=0;
	int ifCount = 0, elseIfCount = 0, elseCount = 0, totalCount = 0;
...
//????
		...
		if(!strcmp("myout.sam",file))contigField1 = strtok(NULL, "\t");
		temp=strtok(NULL,"\t");
		pos1=atoi(temp);
...
//			cout<<it<<endl;
    ...
    cout << ifCount << " + " << elseIfCount << " + " << elseCount << " = " << ifCount + elseIfCount + elseCount << ", " << totalCount << endl;
	fclose(mapFile);
	return logsum;
}
